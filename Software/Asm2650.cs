...
namespace Signetics2650 {
    ...
    class Asm2650 {
        ...
        static void FillInstructionTable() {
            ...
            cpuInstrs.Add("tmi",  new Instruction(0xF4, false, true, AddressingModes.Immediate));
            ...
            cpuInstrs.Add("push", new Instruction(0x11, false, false, AddressingModes.Implicit));
            cpuInstrs.Add("pop", new Instruction(0x10, false, false, AddressingModes.Implicit));
        }
        ...
        private static int ParseDB(string s, Dictionary<string, uint> symbolTable, List<uint> ret) {
            if(s.StartsWith('"')) { //String
                ...
                uint a;
                if(!ParseInstrArg(s, symbolTable, out a)) return 1;
                if(a > 255) return 2;
                ret.Add(a);
            }
            return 0;
        ...
        public static void Assemble(string source) {
            ...
            for(int i = 0; i < lines.Length; i++) {
                ...
                if(line.StartsWith('\t') || line.StartsWith(' ')) {
                    ...
                    if(line.StartsWith("db ") || line.StartsWith("db\t")) {
                        ...
                        uint len = 0;
                        if(line.StartsWith('"')) {
                            if(!line.EndsWith('"')) Console.WriteLine($"Error on line {i + 1}: Invalid formatting.");
                            len += (uint)(line.Length - 2);
                        }else if(line.StartsWith('\'')) {
                            if(line.Length != 3 || line[2] != '\'') Console.WriteLine($"Error on line {i + 1}: Invalid formatting.");
                            len++;
            ...
            for(int i = 0; i < lines.Length; i++) {
                ...
                if(line.StartsWith('\t') || line.StartsWith(' ')) {
                    ...
                    if(line.StartsWith("db ") || line.StartsWith("db\t")) {
                        line = line.Substring(3);
                        List<uint> a = new List<uint>();
                        int res = ParseDB(line, symbolTable, a);
                        if(res != 0) {
                    ...
                    if(instr.isBranch && instr.addrMode == AddressingModes.Relative) {
                        ...
                        int diff = (int)argValue - (int)ptr - 2;
                        if(diff < -64 || diff > 63) {
            ...
            string binFilename = $"{outputFilename}.bin";
            byte[] outputDataBytes = new byte[ptr];
            for(int i = 0; i < ptr; i++) {
                outputDataBytes[i] = (byte)outputData[i];
            }
            File.WriteAllBytes(binFilename, outputDataBytes);
